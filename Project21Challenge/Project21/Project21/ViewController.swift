...
//        let trigger = UNCalendarNotificationTrigger(dateMatching: dateComponents, repeats: true)
    ...
    func registerCategories(){
         ...
         * .destructive將清除本通知。
         * 當User對通知有任何行為時，didReceive函式會收到通知。
         */
        let show = UNNotificationAction(identifier: "show", title: "Tell me more...", options: .foreground)
        let show2 = UNNotificationAction(identifier: "show2", title: "稍後提醒我", options: .destructive)

        /** 產生通知類別物件
         * 在這個通知類別中，我們設計了兩個Action( 兩個按鈕 )。
         * 換句話說，若iOS發現你送出來的通知，類別是"alarm"，
         * 就會有這兩個action按鈕。
         */
    ...
    func showAlert(message: String){
        let ac = UIAlertController(title: "點選通知", message: "你點選了\(message)", preferredStyle: .alert)
